...
class Sun.new(buffer, bodyIndex){
    ...
    method getStorage(){
        return mySun.getStorage()
    }
    
    method offsetMomentum(px,py,pz){
        return mySun.offsetMomentum(px,py,pz)    
    }
}
...
method runtest(n){
        ...
        var bodies := NBodySystem.new([Sun.new(bodybuffer,0), Jupiter.new(bodybuffer,1), Saturn.new(bodybuffer,2), Uranus.new(bodybuffer,3), Neptune.new(bodybuffer,4)])
